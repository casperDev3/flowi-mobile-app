...
- Не показувати порожній стан там, де читання зі сховища ВПАЛО: «не прочиталось» і «порожньо» — різні речі, а автозапис поверх fallback знищує дані назавжди (аудит E2, ERR-01)
- Не вважати стан компонента повною правдою про колекцію в `saveSynced`: запис, що приїхав синком у живу вкладку, отримає тумбстоун і зникне з сервера (ERR/DI-01)
- Не закривати один аркуш і відкривати наступний в одному тіку: `SheetModal` ще тримає перший змонтованим ~150 ms, і лишається невидимий шар, що з'їдає всі дотики до перезапуску (NEW-02)
- Не ставити `maxHeight` у відсотках на аркуш без прокрутного контейнера всередині — кнопка «Зберегти» опиняється за екраном, і форму неможливо зберегти взагалі (NEW-01)
- Не покладатись на `hitSlop` там, де контрол стоїть впритул до краю предка: `hitSlop` розширює ціль лише в межах `bounds` батька, тож `36 + 4` дає не 44, а ті самі 36 (A11Y-08)
- Не ставити `accessible={false}` лише на внутрішню обгортку аркуша: піддерево склеює ЗОВНІШНІЙ бекдроп, і VoiceOver далі бачить один вузол замість форми

## Аудит і прогін на пристрої
Як підняти стенд (сервер, тестовий акаунт, Release-збірка на симулятор),
як ганяти Maestro і які пастки вже відомі — **`docs/AUDIT_RUNBOOK.md`**.
Коротко, щоб не наступити двічі:
- прогін на пристрої — **тільки Release-збіркою**: Debug це дев-клієнт, і `clearState` у Maestro стирає йому адресу Metro
- `MAESTRO_DRIVER_STARTUP_TIMEOUT=300000` обов'язковий, інакше драйвер XCUITest не встигає піднятись
- `react-native-web` доходить лише до авторизації: `expo-secure-store` на вебі це `export default {}`
- контраст рахувати з пікселів скріна, не з токенів — у темній темі `BlurView` віддає не константу
- **ніколи не ганяти аудит на продакшн-акаунті** (`api.flowi.casperdev.site`) — сценарії створюють і видаляють записи незворотно
