...
export default function SharedScreen() {
  ...
  function applyRemoteItem(type: SectionType, remote: any) {
    ...
    setItems(prev => {
      ...
      const idx  = list.findIndex(i => i.local_id === merged.local_id);
  ...
  async function toggleItem(item: LocalItem) {
    await persistItem(activeSection, { ...item, checked: !item.checked, updated_at: new Date().toISOString() });
  }

  async function deleteItem(item: LocalItem) {
    await persistItem(activeSection, { ...item, deleted: true, updated_at: new Date().toISOString() });
  }

  async function saveEditItem() {
    if (!editItem || !editText.trim()) return;
    await persistItem(activeSection, { ...editItem, text: editText.trim(), updated_at: new Date().toISOString() });
  ...
  async function createGroup() {
    ...
    try {
      ...
      Alert.alert('Помилка', 'Не вдалося створити групу. Перевірте підключення.');
    } finally {
      setCreating(false);
    }
  }
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <ScrollView
          ...
          {SECTIONS.map(s => {
            ...
            const pending = items[s.type].filter(i => !i.deleted && !i.checked).length;
            return (
              <TouchableOpacity
                key={s.type}
                ...
                {!active && pending > 0 && (
                  <View style={[st.badge, { backgroundColor: c.accent }]}>
                    <Text style={st.badgeText}>{pending}</Text>
                  </View>
                )}
              </TouchableOpacity>
            );
        ...
        <ScrollView
            ...
            <View style={{ marginBottom: 14 }}>
              ...
              <View style={[st.progressTrack, { backgroundColor: c.dim }]}>
                <View style={[st.progressFill, {
                  ...
                  width: `${visibleItems.length ? (checkedCount / visibleItems.length) * 100 : 0}%` as any,
                }]} />
              </View>
            </View>
          )}
          ...
          {visibleItems.length === 0 && (
            <View style={{ alignItems: 'center', paddingVertical: 56 }}>
              <IconSymbol name={SECTIONS.find(s => s.type === activeSection)!.icon as any} size={48} color={c.sub} />
            ...
            <BlurView intensity={isDark ? 20 : 40} tint={isDark ? 'dark' : 'light'}
              ...
              {visibleItems.map((item, idx) => (
                <View
                  ...
                  <TouchableOpacity
                    style={{ flex: 1 }}
                    onPress={() => { setEditItem(item); setEditText(item.text); setShowEditModal(true); }}>
          ...
          {activeSection === 'shopping' && checkedCount > 0 && (
            <TouchableOpacity
              onPress={() => visibleItems.filter(i => i.checked).forEach(i => deleteItem(i))}
