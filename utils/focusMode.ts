 ...
 *
 * ЛИШЕ на широкому екрані. На телефоні плаваюча кнопка забирає той самий кут,
 * що й власна дія екрана, і висить над кожним списком заради режиму, який на
 * маленькому екрані все одно показує один-два годинники. Там вхід лишається
 * там, де й був до появи цієї кнопки, — у шапці вкладки «Час».
 *
 * На екранах входу її немає й на планшеті: там користувач ще не авторизований,
 * і таймерів у нього немає за визначенням. Список береться з SIDEBAR_HIDDEN_ON
 * НА ЧИТАННЯ — дописувати туди маршрути заборонено, бо та сама константа
 * працює гвардом редіректу гостя.
 */
export function focusButtonVisible(pathname: string, isWide: boolean): boolean {
