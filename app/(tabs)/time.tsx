              ...
              return (
                <BlurView intensity={isDark ? 50 : 70} tint={isDark ? 'dark' : 'light'} style={[s.sheet, { borderColor: c.border, backgroundColor: c.sheet }]}>
                  ...
                  <View style={{ alignItems: 'center', marginBottom: 22 }}>
                    ...
                    <Text style={[s.detailDur, { color: cfg.color, marginTop: 14 }]}>{fmtDur(selected.duration)}</Text>
                    <Text style={[s.detailTask, { color: c.text }]}>{selected.task}</Text>
                  </View>
                  <View style={[s.infoBlock, { borderColor: c.border, backgroundColor: c.dim }]}>
                    <InfoRow icon={cfg.icon} label="Зміна"      value={`${cfg.label} · ${cfg.hours}`} text={c.text} sub={c.sub} border={c.border} last={false} />
                    <InfoRow icon="timer"    label="Тривалість"  value={fmtDur(selected.duration)}      text={c.text} sub={c.sub} border={c.border} last={false} />
                    <InfoRow icon="calendar" label="Дата"        value={new Date(selected.date).toLocaleDateString('uk-UA', { day: 'numeric', month: 'long', year: 'numeric' })} text={c.text} sub={c.sub} border={c.border} last />
                  </View>
