...
/**
 * Чисті калорії за день: різниця між спожитим і спаленим, за модулем.
 *
 * Спалені калорії приходять з Apple Health автоматично, а їжу користувач
 * вносить руками, тож різниця часто виходить відʼємною — і на екрані було
 * `-44 кк`.
 *
 * Свідоме рішення показувати МОДУЛЬ, а не обрізати по нулю: екран показує
 * розмір розбіжності з нормою, а не «скільки з'їдено». Знак при цьому
 * втрачається, тобто дефіцит 44 і профіцит 44 виглядають однаково — це
 * прийнято.
 */
export function calcNetCalories(consumed: number, burned: number): number {
  ...
  return Math.round(Math.abs(safeConsumed - safeBurned));
}
